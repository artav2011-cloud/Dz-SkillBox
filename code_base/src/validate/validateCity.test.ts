...
describe('validateCity - Валидация города', () => {
  ...
  describe('validateCity - полная валидация', () => {
    ...
    test('должен вернуть ошибку при недопустимых символах', () => {
      const result = validateCity('Moscow@123');
      expect(result.isValid).toBe(false);
      expect(result.error).toBe('Название содержит недопустимые символы');
    });
    ...
    test('должен пропускать название с пробелами', () => {
      const result = validateCity('New York');
      expect(result.isValid).toBe(true);
      expect(result.error).toBeNull();
    });

    test('должен пропускать название на кириллице', () => {
